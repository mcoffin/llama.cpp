...
# prompt cache snapshots via the /slots save/restore actions (--save-ram-path)
# ref: https://github.com/ggml-org/llama.cpp/pull/16391
def test_completion_prompt_cache_snapshot():
    global server
    server.n_slots = 1
    server.kv_unified = True
    ...
    server.save_ram_path = "./tmp/ram"
    os.makedirs("tmp/ram", exist_ok=True)
    # same file name in both directories: slot file in tmp/, snapshot in tmp/ram/
    slot_path     = os.path.join("tmp", "prompt_cache_snapshot.bin")
    snapshot_path = os.path.join("tmp", "ram", "prompt_cache_snapshot.bin")
    for path in (slot_path, snapshot_path):
        if os.path.exists(path):
            os.remove(path)

    # two unrelated long prompts, sharing only the " Hello " prefix
    prompt_a = " Hello 1" * 100
    prompt_b = " Hello 2" * 100

    server.start()

    # first prompt is fully processed and stays in the idle slot
    res = server.make_request("POST", "/completion", data={"prompt": prompt_a, "n_predict": 4})
    assert res.status_code == 200
    timings = res.body["timings"]
    n_prompt_a = timings["prompt_n"] + timings["cache_n"]
    assert timings["cache_n"] == 0
    content_a = res.body["content"]

    # the second unrelated prompt displaces the first into the RAM cache and is processed from scratch
    res = server.make_request("POST", "/completion", data={"prompt": prompt_b, "n_predict": 4})
    assert res.status_code == 200
    timings = res.body["timings"]
    n_prompt_b = timings["prompt_n"] + timings["cache_n"]
    assert timings["cache_n"] == 0
    assert timings["prompt_n"] == n_prompt_b
    content_b = res.body["content"]

    # explicit save writes the prompt cache snapshot next to the slot file
    res = server.make_request("POST", "/slots/0?action=save", data={"filename": "prompt_cache_snapshot.bin"})
    assert res.status_code == 200
    assert "ram" in res.body
    assert res.body["ram"]["n_entries"] >= 2
    assert res.body["ram"]["n_bytes"] > 0
    assert os.path.exists(snapshot_path)
    assert os.path.getsize(snapshot_path) > 0

    # restart: no automatic save at shutdown or load at startup
    server.stop()
    server.start()

    # restore the snapshot into the RAM cache, the slot is restored first
    res = server.make_request("POST", "/slots/0?action=restore", data={"filename": "prompt_cache_snapshot.bin"})
    assert res.status_code == 200
    assert "ram" in res.body
    assert res.body["ram"]["n_entries"] >= 2

    # both prefixes are reused from the restored snapshot
    res = server.make_request("POST", "/completion", data={"prompt": prompt_a, "n_predict": 4})
    assert res.status_code == 200
    timings = res.body["timings"]
    assert timings["cache_n"] > n_prompt_a*0.9  # only the divergent suffix is processed
    assert timings["prompt_n"] == n_prompt_a - timings["cache_n"]
    assert res.body["content"] == content_a

    res = server.make_request("POST", "/completion", data={"prompt": prompt_b, "n_predict": 4})
    assert res.status_code == 200
    timings = res.body["timings"]
    assert timings["cache_n"] > n_prompt_b*0.9
    assert timings["prompt_n"] == n_prompt_b - timings["cache_n"]
    assert res.body["content"] == content_b

    server.stop()


# a malformed snapshot must not corrupt the cache or prevent a later valid save
def test_completion_prompt_cache_snapshot_malformed():
    global server
    server.n_slots = 1
    server.kv_unified = True
    server.cache_ram = 512
    ...
    os.makedirs("tmp/ram", exist_ok=True)
    # same file name in both directories: slot file in tmp/, snapshot in tmp/ram/
    slot_path     = os.path.join("tmp", "prompt_cache_snapshot_bad.bin")
    snapshot_path = os.path.join("tmp", "ram", "prompt_cache_snapshot_bad.bin")
    with open(snapshot_path, "wb") as f:
        f.write(b"\x00" * 32)  # invalid snapshot
    if os.path.exists(slot_path):
        os.remove(slot_path)

    prompt = " Hello 1" * 100

    server.start()  # startup no longer touches the snapshot file

    # cache miss, no partial state from the malformed file
    ...
    # save a valid slot file, then corrupt only the prompt cache snapshot
    res = server.make_request("POST", "/slots/0?action=save", data={"filename": "prompt_cache_snapshot_bad.bin"})
    assert res.status_code == 200
    assert "ram" in res.body
    with open(snapshot_path, "wb") as f:
        f.write(b"\x00" * 32)  # invalid snapshot again

    # restore fails with 400 on the malformed snapshot, the slot stays restored
    res = server.make_request("POST", "/slots/0?action=restore", data={"filename": "prompt_cache_snapshot_bad.bin"})
    assert res.status_code == 400
    assert "snapshot" in res.body["error"]["message"]

    # a fresh prompt is still a cache miss: no partial state from the failed load
    res = server.make_request("POST", "/completion", data={"prompt": " Hello 2" * 100, "n_predict": 4})
    assert res.status_code == 200
    timings = res.body["timings"]
    assert timings["cache_n"] == 0

    # saving again replaces the corrupted snapshot with a valid one
    res = server.make_request("POST", "/slots/0?action=save", data={"filename": "prompt_cache_snapshot_bad.bin"})
    assert res.status_code == 200
    assert "ram" in res.body
    assert os.path.exists(snapshot_path)
    with open(snapshot_path, "rb") as f:
        assert f.read(4) == b"SRPC"
